...
def parse_column(self, line, state):
    ...
    #show create table 순서대로 column 리스트를 만들고 있기 때문에 column position은 순서대로이다.
    column_index = len(state.columns)-1
    state.columns[column_index]["ordinalPosition"] = column_index+1
