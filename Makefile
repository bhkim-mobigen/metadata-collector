...
# 	pytest --screenshot=only-on-failure --output="tests/e2e/artifacts" $(ARGS) --slowmo 5 --junitxml=junit/test-results-e2e.xml tests/e2e
#
# ## Yarn
...
py_antlr:  ## Generate the Python code for parsing FQNs
	antlr4 -Dlanguage=Python3 -o src/metadata/generated/antlr ${PWD}/spec/src/main/antlr4/org/openmetadata/schema/*.g4

.PHONY: js_antlr
js_antlr:  ## Generate the Python code for parsing FQNs
	antlr4 -Dlanguage=JavaScript -o openmetadata-ui/src/main/resources/ui/src/generated/antlr ${PWD}/spec/src/main/antlr4/org/openmetadata/schema/*.g4
...
install_antlr_cli:  ## Install antlr CLI locally
	@if [ -f /usr/local/bin/antlr4 ]; then \
		echo "antlr4 already installed"; \
	else \
		mkdir -p /tmp/antlr && \
...
# 	docker build -t openmetadata-ingestion:scan -f Dockerfile.ci .
# 	snyk container test openmetadata-ingestion:scan --file=Dockerfile.ci $(SNYK_ARGS) --json > security-report/ingestion-docker-scan.json | true;
...
# 	docker build -t openmetadata-ingestion-base-slim:scan -f operators/docker/Dockerfile.ci --build-arg INGESTION_DEPENDENCY=slim .
# 	snyk container test openmetadata-ingestion-base-slim:scan --file=operators/docker/Dockerfile.ci $(SNYK_ARGS) --json > security-report/ingestion-docker-base-slim-scan.json | true;
#
# .PHONY: snyk-report
...
# 	docker build -f operators/docker/Dockerfile.ci . -t openmetadata/ingestion-base:local
#
# .PHONY: build-ingestion-base-slim-local
# build-ingestion-base-local:  ## Builds the ingestion DEV docker operator with the local ingestion files
# 	$(MAKE) install_dev generate
# 	docker build -f operators/docker/Dockerfile.ci . -t openmetadata/ingestion-base-slim:local --build-arg INGESTION_DEPENDENCY=slim
#
...
# 	@file_paths="docker/docker-compose-docker-compose-ingestion.yml \
